...
var npcs = map[int]*NPC{
	1: &NPC{
		...
		script: []NPCScriptStage{
			...
			{op: opWait, duration: 3 * time.Second},
			{op: opSay, text: "Try it right now! I'll wait for you. (Enter the command: look)"},
			...
			{op: opWait, duration: 5 * time.Second},
			{op: opSay, text: "Wanna know more about the look command? Go on, try `help look`! I'll wait."},
			...
			{op: opWait, duration: 2 * time.Second},
			{op: opSay, text: "Go `north` once, and I'll direct you futher."},
			...
			{op: opSayDiffRoom, text: "If you wish to skip the tutorial, simply go `north` till you end up in the Welcome Plaza!"},
		},
	},
	2: &NPC{
		id:         2,
		...
		script: []NPCScriptStage{
			...
			{op: opWait, duration: 3 * time.Second},
			{op: opSay, text: "Take a look with `look` just in case you didn't notice. I'll wait."},
			{op: opWaitCommand, command: []string{"look", "l"}},
			{op: opWait, duration: 2 * time.Second},
			...
			{op: opWait, duration: 5 * time.Second},
			{op: opSay, text: "Which means, you can do `pickup x` where x is any number between 0 & 2."},
			{op: opWait, duration: 3 * time.Second},
			...
			{op: opWait, duration: 5 * time.Second},
			{op: opSay, text: "`pickup 2` would pickup the 3rd item, which would be Item B!"},
			{op: opWait, duration: 5 * time.Second},
			...
			{op: opWait, duration: 6 * time.Second},
			{op: opSay, text: "Why don't you try pickup up a wooden sword with the keyword method? I'll wait."},
			...
			{op: opWait, duration: 2 * time.Second},
			{op: opSay, text: "Good job. Now, use numbers, and pickup specifically the 2nd Broken Wooden Sword. Remember, indexes start from 0! I'll be waiting."},
			...
			{op: opSay, text: "You might need to move around a bit to find your target."},
	...
	3: &NPC{
		...
		script: []NPCScriptStage{
			...
			{op: opWait, duration: 4 * time.Second},
			{op: opSay, text: "That's all I had for you."},
			{op: opWait, duration: 2 * time.Second},
			{op: opSay, text: "Go `east` twice to talk to Cedriah about her boring economics..."},
			{op: opWait, duration: 4 * time.Second},
			{op: opSay, text: "Or, you can just skip her and read `help merchants` instead. Wink."},
			{op: opWait, duration: 5 * time.Second},
			{op: opSay, text: "If you're done, then go `east` once, then go `north` till you hit the `Welcome Plaza`."},
			{op: opWait, duration: 6 * time.Second},
	...
	4: &NPC{
		...
		script: []NPCScriptStage{
			...
			{op: opWait, duration: 4 * time.Second},
			{op: opSay, text: "Take a `look`, I'll wait."},
			...
			{op: opWait, duration: 3 * time.Second},
			{op: opSay, text: "You use the `list <entity>` command, without the <>, where entity is a word from the entity's name."},
			{op: opWait, duration: 5 * time.Second},
			{op: opSay, text: "It's just like you learnt at the start, except numbers don't work on entities!"},
			{op: opWait, duration: 3 * time.Second},
			...
			{op: opWait, duration: 6 * time.Second},
			{op: opSay, text: "They've become insanely strong. Not strong enough for normal people like us to fathom."},
			{op: opWait, duration: 4 * time.Second},
			{op: opSay, text: "But perhaps, one day... You might be strong enough."},
			{op: opWait, duration: 4 * time.Second},
			...
			{op: opWait, duration: 3 * time.Second},
			{op: opSay, text: "Then, go on. Buy the Wooden Stick that the merchant is selling!"},
			{op: opWait, duration: 5 * time.Second},
			{op: opWaitCommand, command: []string{"buy 20 dummy", "buy 20 merchant"}},
			{op: opWait, duration: 2 * time.Second},
			{op: opSay, text: "Hey! You did it! Now, sell the same item back to the merchant."},
			{op: opWait, duration: 3 * time.Second},
			{op: opSay, text: "Use the same command, but replace buy with sell this time!"},
			{op: opWait, duration: 3 * time.Second},
	...
	5: &NPC{
		...
		script: []NPCScriptStage{
			...
			{op: opSay, text: "That's all I had to teach you! Good luck out there, kiddo!"},
			{op: opWait, duration: 2 * time.Second},
		},
	},
	6: &NPC{
		...
		script: []NPCScriptStage{
			...
			{op: opWait, duration: 4 * time.Second},
			{op: opSay, text: "I'm gonna keep this short and sweet. Cya out there!"},
			{op: opWait, duration: 4 * time.Second},
		},
	},
}
