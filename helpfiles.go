...
var HelpEntries = []HelpEntry{
	...
	},
	{
		...
		text: []string{
			...
			"- uneq, uneq - Unequips a worn item from the slot",
			"    specified",
			"- fight, attack, f - Initiates a fight with a player",
			"    or mob",
			"- train - Increases a selected stat by the given",
			"    number of trains",
	...
	},
	{
		...
		category:    "Command",
		text: []string{
			"This command shows you whatever you see in the room",
			"that you are in right now!",
			"",
			"Generally, the output of `look` looks like this:",
			"",
			"Room 1 Name",
			"  Room 1 Description",
			"",
			...
			"  ! NPC Name",
			"",
			"The number before the name of the item",
			"signifies quantity.",
			"As a general rule, all player names have",
			"a * before them.",
			"While all NPCs have a ! before their names.",
		},
	},
	{
		id:          30,
		title:       "Clans",
		keywords:    []string{"clan", "clans", "clansystem"},
		instruction: []string{},
		category:    "Topic",
		text: []string{
			"The Forlorn clan system allows you to form 'clans'",
			"which are essentially just groups of players.",
			"You can then enjoy the game together with your",
			"clanmates!",
			"",
			"You can only be in one clan at a time!",
			"",
			"Important concepts:",
			"> Clan status: Status can either be open or closed.",
			"  open meaning ANYONE who knows the name of the clan",
			"  can join it, while closed means NO ONE can join it.",
			"  Only the leader can change the clan status.",
			"",
			"Below is a detailed explanation for all clan related",
			"commands.",
			"",
			"- clan info: Shows a detailed clan profile card. Shows",
			"  a list of members, clan name, et cetera.",
			"- clan top: Prints a formatted list of ALL clans",
			"  arranged in descending order of amount of members.",
			"  (You can use `clan top` to find clans to join!)",
			"- clan join <name>: Joins the clan with the specified",
			"  name, only if the clan is open!",
			"- clan say <msg>: Sends a message in the clan chat!",
			"- clan leave: Leaves your current clan.",
			"",
			"OWNERS ONLY-",
			"- clan create <name>: Creates a clan called 'name',",
			"  where 'name' is whatever you specified. You",
			"  need to pay 50,000 gold to create a clan.",
			"- clan <open|close>: Changes the clan status to either",
			"  open or closed.",
			"- clan tag <tag1>: Where 'tag1' is any tag you specify!",
			"  tags MUST be 4 characters long.",
			"- clan kick <player_name>: Kicks a player from your clan.",
